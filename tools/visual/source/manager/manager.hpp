...
namespace itl
{
    ...
    class Manager
    {
       ...
       public:
        explicit Manager(std::shared_ptr<Logger> log);
        virtual bool load_data(const std::string& path_to_data,
                               int number_of_copies) = 0;
        virtual bool update_single(const std::string& path_to_data,
                                   int idx_copy) = 0;
        int size() const;
    ...
    template <class T>
    Manager<T>::Manager(std::shared_ptr<Logger> log) : logger(log)
    {
    }
    ...
    T* Manager<T>::get(int idx_copy, int idx_data)
    {
        if(this->storage[idx_copy].first != this->data_paths[idx_data])
        {
            ...
            {
                this->logger->log(constants::manager::failed_update,
    ...
    Manager<T>::Manager()
    {
        this->logger->log(std::string(constants::info::init_module_msg_start) +
                              std::string(typeid(this).name()),
                          Logger::STREAM::CONSOLE, Logger::TYPE::INFO);

        this->logger->log(std::string(constants::info::init_module_msg_end) +
                              std::string(typeid(this).name()),
                          Logger::STREAM::CONSOLE, Logger::TYPE::INFO);
    }
}  // namespace itl